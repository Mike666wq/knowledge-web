...
- 进程是**程序关于某个数据集合的一次运行活动**，是操作系统进行**资源分配和调度的基本单位**。

- 简单来说，当在系统中执行一个程序（如 `/usr/sbin/httpd`）时，操作系统会为其创建一个进程。这个进程是程序代码的运行实例，拥有独立的内存、文件句柄和安全属性，并会经历创建、运行、等待、终止等生命周期。

------

### 🆚 进程 vs. 程序

- 进程与程序是两个紧密相关但本质不同的概念，主要区别如下：

| 特性         | 程序 (Program)                                         | 进程 (Process)                                           |
| ------------ | ------------------------------------------------------ | -------------------------------------------------------- |
| **本质**     | 存储在磁盘上的**静态二进制文件**，是指令和数据的集合。 | 程序在内存中的一次**动态执行过程**。                     |
| **生命周期** | 可以长期存在，除非被手动删除。                         | 有严格的生命周期：创建 → 运行 → 终止。                   |
| **资源占用** | 本身不占用系统运行时资源（CPU、内存等）。              | 运行时需占用独立的地址空间、内存、文件描述符等系统资源。 |
| **并发性**   | 概念上无法描述并发。                                   | 多个进程可以并发执行，是系统并发执行的基本单位。         |
| **对应关系** | 一个程序可被加载为多个进程。                           | 一个进程只对应一个程序代码，但会处理不同的数据。         |

- **示例：**

- `/usr/sbin/sshd`、`/usr/local/nginx/sbin/nginx`这些都是程序文件。当启动它们时，系统会为每个实例创建一个独立的进程。
...
3. **栈帧 (Stack)**
   ...
   存放函数调用的局部变量、返回地址等，随函数调用动态增长和收缩。

4. **堆 (Heap)**

   用于动态内存分配（如 `malloc`），进程可请求和释放。

5. **内存地址空间**

   由上述各部分组成的虚拟内存区域，进程间相互隔离。

6. **内核数据结构 (PCB)**

   操作系统为管理进程而创建的**进程控制块 (Process Control Block)**，是进程存在的唯一标志。它记录了进程ID (PID)、状态、优先级、寄存器上下文、打开的文件、内存映射等信息。

7. **安全属性**

   包括进程所有者 (UID/GID) 和权限，决定了其对系统资源的访问能力。

8. **执行线程 (Threads)**

   现代操作系统中的进程是**线程的容器**。一个进程至少包含一个主线程，也可以创建多个线程来并发执行任务，共享进程的资源。
...
1. **动态性**
   ...
   进程是程序执行的动态过程，有创建、运行、终止的生命周期，而非静态实体。

2. **并发性**

   多个进程可以在宏观上同时运行，是操作系统实现并发执行的基础。

3. **独立性**

   每个进程拥有独立的地址空间和资源，一个进程的崩溃通常不会直接影响其他进程。

4. **异步性**

   各进程按各自独立的、不可预知的速度向前推进，需要同步机制来保证结果的正确性。

5. **结构性**

   每个进程都由一个进程控制块 (PCB)、程序段和数据段构成，操作系统通过PCB来感知和管理进程。

------

### 🔄 进程的状态

操作系统为了管理进程，会定义几种基本状态，进程在其生命周期中会在这些状态间转换：

- **就绪 (Ready)**：进程已获得除CPU外的所有必要资源，等待被调度执行。
- **运行 (Running)**：进程正在CPU上执行。
- **阻塞/等待 (Blocked/Waiting)**：进程因等待I/O操作或某个事件完成而暂停，即使分配CPU也无法运行。

**状态转换示例：**

- `就绪`→ `运行`(被调度选中) → `阻塞`(发起I/O请求) → `就绪`(I/O完成) → `运行`...

# 2. 进程类型

- 操作系统中，进程通常可以从不同维度进行分类。以下是几种常见的划分方式：

### 🖥️ 按功能与交互方式

- 这是 Linux/Unix 系统中最常见的分类方法。

| 类型           | 主要特点                                                     | 示例                            |
| -------------- | ------------------------------------------------------------ | ------------------------------- |
| **交互进程**   | 由 Shell 启动，需与用户交互，可运行在前台或后台。            | 在终端执行 `vim`、`top`等命令。 |
| **批处理进程** | 无需终端交互，按预设顺序执行一系列任务，常与 `at`、`cron`关联。 | 系统自动执行的备份脚本。        |
| **守护进程**   | 在系统启动时运行，常驻后台，无控制终端，提供持续服务。       | `sshd`、`httpd`、`crond`。      |
...
- **系统进程 (内核线程)**
  - **运行者**：以 `root`等特权用户身份运行。
  - **职责**：管理内存、调度进程、处理中断等核心系统任务。
  - **特点**：通常不可被普通用户干预。
- **用户进程**
  - **运行者**：由普通用户启动。
  - **职责**：运行各类应用程序（如浏览器、办公软件）。
  - **特点**：受权限限制，其资源访问受控于系统策略。

------

### ⚙️ 按运行状态与生命周期

这种分类侧重于进程在生命周期中所处的状态。

- **前台进程**

  占用终端，直接与用户交互。例如，在终端前台运行的 `ping`命令。

- **后台进程**

  在后台运行，不占用终端，用户可继续输入其他命令。通常在命令末尾加 `&`启动，如 `tar czf backup.tar.gz /data &`。

- **守护进程 (Daemon)**

  一种特殊的后台进程，独立于任何终端，在系统启动时初始化，并持续运行以提供特定服务。

- **孤儿进程 (Orphan Process)**

  父进程先于子进程终止，子进程会被 `init`进程（PID为1）接管。

- **僵尸进程 (Zombie Process)**

  子进程已终止，但其退出状态未被父进程回收。它不再占用CPU和内存，但仍在进程表中保留一个条目，需由父进程调用 `wait()`来清理。

# 3. 进程的生命周期

![img](18.进程管理/WPYNtDaMQ3fkYeQs.png)

- 父进程复制自己的地址空间(fork)创建一个新的(子)进程结构。每个新进程分配一个唯一的进程ID(PID)，满足跟踪安全性之需。PID和父进程ID(PPID)是子进程环境的元素，任何进程都可以创建子进程，所有进程都是第一个系统进程的后代。
- 所有的任务都是`systemd`这个父进程的子进程
...
- systemd 是 Linux 系统的**初始化系统 (init system)** 和服务管理器，作为系统的**第一个用户空间进程 (PID 1)** 运行。它取代了传统的 System V init (SysVinit)，旨在克服其启动慢、脚本复杂等缺点，并统一管理系统的启动、服务、日志、挂载点等。

> **核心定位**：一个运行在用户空间的、功能强大的系统与服务管理器，是现代 Linux 发行版的事实标准。

------

### 🚀 核心优势与设计特点

- 相较于传统的 SysVinit，systemd 的主要优势在于：

1. **并行启动，速度更快**

   通过构建服务依赖图，尽可能并行启动服务，而非 SysVinit 的串行执行，显著缩短系统启动时间。

2. **基于依赖关系管理**

   自动分析并管理系统组件间的依赖，确保服务按正确的顺序启动和停止，无需手动维护复杂的启动脚本。

3. **按需 (Socket) 激活**

   采用“Socket 激活”和“D-Bus 激活”机制。服务仅在首次收到请求时才被真正启动，这既节省了系统资源，也加快了启动初期的响应速度。

4. **强大的进程追踪**

   利用 Linux **cgroups** 技术来跟踪进程，即使子进程 `fork`多次，也能确保它们始终处于 systemd 的管理之下，便于资源控制和状态监控。

5. **统一的管理接口**

   提供一套统一的命令（如 `systemctl`, `journalctl`）来管理系统的各个方面，取代了 `service`、`chkconfig`及分散的日志文件等多种工具。

------

### 🏗️ 核心概念：Unit (单元)

- systemd 将所有需要管理的对象都抽象为 **Unit (单元)**，并通过文本格式的配置文件进行定义。常见的 Unit 类型包括：

| Unit 类型     | 文件后缀     | 功能说明                                                     |
| ------------- | ------------ | ------------------------------------------------------------ |
| **Service**   | `.service`   | 管理系统服务（如 nginx, sshd），定义如何启动、停止、重启等。 |
| **Target**    | `.target`    | 用于逻辑分组，替代传统的“运行级别”（Runlevel），如 `multi-user.target`。 |
| **Mount**     | `.mount`     | 管理系统文件系统的挂载点，可替代 `/etc/fstab`的部分功能。    |
| **Automount** | `.automount` | 定义文件系统的自动挂载点。                                   |
| **Socket**    | `.socket`    | 监听 socket，用于实现服务的按需激活。                        |
| **Timer**     | `.timer`     | 定义定时任务，可替代传统的 `cron`功能。                      |
| **Path**      | `.path`      | 监控文件或目录的变化，并触发其他 Unit。                      |
| **Swap**      | `.swap`      | 管理交换分区或交换文件。                                     |
| **Slice**     | `.slice`     | 用于对进程进行资源分组和限制。                               |
| **Scope**     | `.scope`     | 用于管理由 systemd 外部启动的进程。                          |
| **Snapshot**  | `.snapshot`  | 用于创建和恢复系统状态的快照。                               |

------

### 💻 systemd 的典型应用

在 RHEL/CentOS 7+、Rocky Linux 8/9 及 Ubuntu 16.04+ 等系统中，systemd 是默认的初始化系统。您可以通过 `ps -p 1`命令查看，PID 为 1 的进程即为 `systemd`。

**1. 管理服务**

使用 `systemctl`命令进行服务的生命周期管理：
...
`systemctl`也集成了对系统关机和重启等操作的统一管理：
...
使用 `journalctl`命令查看由 `journald`服务统一收集的系统日志：
...
- systemctl 通过放在`/usr/lib/systemd/system/xxxx.service` 配置文件，实现管理服务的任务(yum安装的服务自带配置)
- systemctl 服务的配置文件格式

| systemctl配置格式 |                                                              |
| ----------------- | ------------------------------------------------------------ |
| [Unit]            | 说明服务的作用，服务的依赖                                   |
| [Service]         | 核心部分：指定服务开启、关闭、重启分别使用的命令，服务的一些环境变量 |
| [Install]         | 指定运行级别                                                 |
...
After=network.target sshd-keygen.target				# 这个服务需要在这些服务启动之后才能启动(依赖)
Wants=sshd-keygen.target							# 也是需要依赖的服务

[Service]
Type=notify											# 服务类型  notify  oneshot forking
...
KillMode=process									# 服务关闭的方式，建议使用ExecStop
Restart=on-failure									# 服务重启的规则
RestartSec=42s										# 服务重启的间隔

[Install]
WantedBy=multi-user.target							# 运行级别，多用户

[root@localhost ~]# systemctl cat nginx
...
[Service]
Type=forking										# nginx会拉起多个进程，使用forking
PIDFile=/run/nginx.pid
# Nginx will fail to start if /run/nginx.pid already exists but has the wrong
# SELinux context. This might happen when running `nginx -t` from the cmdline.
# https://bugzilla.redhat.com/show_bug.cgi?id=1268621
ExecStartPre=/usr/bin/rm -f /run/nginx.pid			# 启动nginx之前执行的命令
...
[root@localhost ~]# cat /run/sshd.pid
804
[root@localhost ~]# ps -aux | grep 804
root         804  0.0  0.2  15860  8960 ?        Ss   09:57   0:00 sshd: /usr/sbin/sshd -D [listener] 0 of 10-100 startups
root        3901  0.0  0.0   3876  2048 pts/0    S+   13:10   0:00 grep --color=auto 804
[root@localhost ~]# pgrep -l sshd
804 sshd
3483 sshd
3487 sshd
...
[![img](18.进程管理/进程优先级.png)](https://git.inmind-lab.com/aaronxu/Linux-book/src/branch/main/01.Linux基础/08.进程管理/进程优先级.png)
...
[root@localhost ~]# ps aux |grep ping
root        4571  0.0  0.0   5092  2560 ?        S    13:43   0:00 ping qq.com
root        4606  0.0  0.0   3876  2048 pts/0    S+   13:44   0:00 grep --color=auto ping
[root@localhost ~]# taill -f nohup.out
-bash: taill: command not found
[root@localhost ~]# tail -f nohup.out
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=73 ttl=128 time=37.3 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=74 ttl=128 time=37.7 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=75 ttl=128 time=37.6 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=76 ttl=128 time=36.7 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=77 ttl=128 time=37.5 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=78 ttl=128 time=37.7 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=79 ttl=128 time=39.6 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=81 ttl=128 time=39.1 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=82 ttl=128 time=37.7 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=83 ttl=128 time=36.8 ms
64 bytes from 123.150.76.218 (123.150.76.218): icmp_seq=84 ttl=128 time=37.5 ms
^C
