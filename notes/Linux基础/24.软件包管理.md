...
Linux 系统中的软件包主要分为源码包和二进制包两大类。

| 类型         | 特点                                                         | 优缺点                                                       |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **源码包**   | 包含程序的原始代码，需编译后才能安装。常见格式：`.tar.gz`, `.tar.bz2`, `.tar.xz`。 | **优点**：可定制功能、优化性能、便于学习源码。 **缺点**：安装步骤繁琐、耗时长、对新手不友好。 |
| **二进制包** | 已编译好的可直接执行的程序。常见格式：RedHat/CentOS 系的 **RPM**；Debian/Ubuntu 系的 **DEB**。 | **优点**：安装简单快捷、管理方便。 **缺点**：功能固定、可能存在依赖问题。 |

**注意**：无论是源码包还是二进制包，安装时都可能存在依赖关系，需要额外安装其他软件包才能正常工作
...
| -------- | --------------------------------------- | ----------------------------------------------------------- |
| **定位** | RHEL 8 及之前版本的默认包管理器。       | YUM 的下一代替代品，从 RHEL 8 开始成为默认。                |
| **实现** | 基于 Python 2，性能和依赖解析能力有限。 | 采用更先进的 `libsolv`依赖解析器，性能更高，基于 Python 3。 |
| **现状** | 在 RHEL 9 中已被完全淘汰。              | 当前主流工具，支持模块化、事务回滚等高级特性。              |

**Rocky Linux 中 yum 命令的本质**

在 Rocky Linux 9 中，`yum`命令本质上是 `dnf`的一个软链接，以保证对旧脚本的兼容性。

```sh
[root@localhost ~]# ll /usr/bin/ | grep dnf
...
rocky-addons.repo  rocky-devel.repo  rocky-extras.repo  rocky.repo  zabbix-agent2-plugins.repo  zabbix.repo
...
Last metadata expiration check: 0:10:19 ago on Mon Mar 30 20:44:09 2026.
...
Last metadata expiration check: 0:11:13 ago on Mon Mar 30 20:44:09 2026.
...
Temporary output repo path: /var/ftp/update/.repodata/
...
![image-20260330220031791](24.软件包管理/image-20260330220031791.png)
...
baseurl=ftp://192.168.73.136/update
gpgcheck=0
enabled=1   
...
```bash
[root@localhost ~]# rpm -ivh httpd-2.4.6-93.el7.centos.x86_64.rpm
- httpd：包名
- 2：主版本号
- 4：次版本号
- 6：修订次数，指的是第几次修改bug
