...
![image-20250327185052680](08-compose-images/image-20250327185052680.png)
...
![image-20250327193919238](08-compose-images/image-20250327193919238.png)
...
![image-20250327195141708](08-compose-images/image-20250327195141708.png)
...
![image-20250327195804039](08-compose-images/image-20250327195804039.png)

二、环境检查

![image-20250327195830286](08-compose-images/image-20250327195830286.png)

三、需要进入到mysql容器中，手动创建一个blog的库然后如下填写

![image-20250327200047631](08-compose-images/image-20250327200047631.png)

四、安装完成

![image-20250327200608550](08-compose-images/image-20250327200608550.png)

五、优化网站

![image-20250327200703348](08-compose-images/image-20250327200703348.png)

----------------

# Dockge管理

- 下载`louislam/dockge`镜像

```bash
root@localhost ~]# mkdir -p /opt/stacks /opt/dockge
[root@localhost ~]# cd /opt/dockge/
[root@localhost dockge]# vim compose.yaml
services:
  dockge:
...
[root@localhost dockge]# docker compose up -d
[+] up 2/2
...
![image-20260716171008346](08.docker-compose单机编排/image-20260716171008346.png)

- 可以直接填入compose.yml文件直接识别容器组

![image-20260716171205669](08.docker-compose单机编排/image-20260716171205669.png)

```bash
version: '3.8'  # 推荐版本，兼容所有常用功能

# 所有服务共享同一个默认网络，直接用服务名通信，不需要--link
services:
  # ========== 1. Redis 服务（和你原有配置完全一致） ==========
  redis:
    image: redis:7.4  # 保留你原来的7.4版本
    container_name: redis
    restart: always
    command: redis-server --save 60 1 --loglevel warning  # 保留你原有的持久化参数
    volumes:
      - /opt/lnmp/redis/config:/usr/local/etc/redis  # 挂载自定义配置
      - /opt/lnmp/redis/data:/data  # 持久化数据目录
    # 健康检查，替代原有的sleep 3，更可靠
    healthcheck:
  ...
  # ========== 2. MySQL 服务（和你原有配置完全一致） ==========
  mysql:
    image: mysql:5.7  # 保留你原来的5.7版本
    container_name: mysql
    restart: always
    environment:
      MYSQL_ROOT_PASSWORD: 123456  # 保留你原有的密码，生产环境请修改
    volumes:
      - /opt/lnmp/mysql/config:/etc/mysql/conf.d  # 挂载自定义配置
      - /opt/lnmp/mysql/data:/var/lib/mysql  # 持久化数据目录
    # 健康检查，等服务完全启动后再启动依赖它的服务
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "localhost", "-uroot", "-p123456"]
      interval: 5s
      timeout: 3s
      retries: 10
      start_period: 30s  # 给MySQL足够的启动时间

  # ========== 3. PHP-FPM 节点01（和原有配置一致） ==========
  php-fpm01:
    image: myphp:7.4-fpm  # 你用自己构建的PHP镜像
    container_name: php-fpm01
    restart: always
    # 依赖mysql和redis，等服务健康再启动
    depends_on:
      mysql:
      ...
      redis:
        condition: service_healthy
    volumes:
      - /opt/lnmp/blog/www:/usr/share/nginx/html  # 共享网站代码
      - /opt/lnmp/blog/php:/usr/local/etc  # PHP自定义配置（比如php.ini）
    # 网络内直接用服务名访问mysql和redis，不需要--link
    # 比如php代码里连接数据库，host填mysql即可，填redis即可

  # ========== 4. PHP-FPM 节点02（和01完全一致，负载用） ==========
  php-fpm02:
    image: myphp:7.4-fpm
    container_name: php-fpm02
    restart: always
    depends_on:
      mysql:
        condition: service_healthy
      redis:
        condition: service_healthy
    volumes:
      - /opt/lnmp/blog/www:/usr/share/nginx/html
      - /opt/lnmp/blog/php:/usr/local/etc

  # ========== 5. 内层负载均衡 lb02（分发给两个PHP-FPM） ==========
  lb02:
    image: nginx:latest  # 你原来用的latest，建议生产环境指定具体版本比如nginx:1.25.4
    container_name: lb02
    restart: always
    depends_on:
      - php-fpm01
      - php-fpm02
    volumes:
      - /opt/lnmp/lb02:/etc/nginx  # 内层负载的Nginx配置
    # 网络内直接用php-fpm01、php-fpm02访问，不需要--link
    # 你的Nginx配置里upstream写 php-fpm01:9000 php-fpm02:9000 即可，完全兼容原有配置

  # ========== 6. Web节点01/02/03（三个Nginx节点） ==========
  web01:
    image: nginx:latest
    ...
    depends_on:
      - lb02
    volumes:
      - /opt/lnmp/blog/nginx:/etc/nginx  # Web节点的Nginx配置
      - /opt/lnmp/blog/www:/usr/share/nginx/html  # 网站代码
  ...
  lb01:
    ...
    depends_on:
      ...
      - web03
    ports:
      - "80:80"  # 只对外暴露这一个端口，和你原有配置一致
    volumes:
      - /opt/lnmp/lb01:/etc/nginx  # 外层负载的Nginx配置
    # 网络内直接用web01、web02、web03访问，不需要--link
    # 你的Nginx配置里upstream写 web01:80 web02:80 web03:80 即可
```

![image-20260716171326719](08.docker-compose单机编排/image-20260716171326719.png)

- 可以看到这个LNMP项目之直接以docker compose在这个图形化界面就可以跑起来了

![image-20260716171437076](08.docker-compose单机编排/image-20260716171437076.png)
