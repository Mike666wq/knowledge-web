...
```bash
[root@localhost ~]# docker run -it --rm  --cpus 2 --cpuset-cpus 0,2 --name test1 polinux/stress-ng:latest --vm 1 --cpu 4
...
```bash
[root@localhost ~]# docker run -it --rm  -d --cpus 1 --cpuset-cpus 0 --cpu-shares 1024 --name test1 polinux/stress-ng:latest --cpu 1 
[root@localhost ~]# docker run -it --rm  -d --cpus 1 --cpuset-cpus 0 --cpu-shares 2048 --name test2 polinux/stress-ng:latest --cpu 1 
...
- 拉取容器压测工具镜像
```bash
[root@docker-server1 ~]# docker pull polinux/stress-ng:latest
[root@docker-server1 ~]# docker run -it --rm polinux/stress-ng:latest -help
```

- 使用压测工具开启两个工作进程，每个工作进程最大允许使用内存256M，且宿主机不限制当前容器的最大内存

```bash
[root@docker-server1 ~]# docker run -it --rm -m 256m --name test1 polinux/stress-ng:latest --vm 2 --vm-bytes 256m
stress-ng: info: [1] defaulting to a 86400 second run per stressor
...
```bash
[root@docker-server1 ~]# docker run -it --rm -m 256m --name test2 polinux/stress-ng:latest --vm 2 --vm-bytes 256m
[root@docker-server1 ~]# docker stats
CONTAINER ID   NAME      CPU %     MEM USAGE / LIMIT   MEM %     NET I/O     BLOCK I/O         PIDS
...
```bash
[root@docker-server1 ~]# docker run -it --rm -m 256m --memory-reservation 128m --name test1 polinux/stress-ng:latest --vm 2 --vm-bytes 256m
[root@docker-server1 ~]# docker stats
CONTAINER ID   NAME      CPU %     MEM USAGE / LIMIT   MEM %     NET I/O     BLOCK I/O         PIDS
...
```bash
[root@docker-server1 ~]# docker run -it --rm -m 256m --memory-swap 512m --name test1 polinux/stress-ng:latest --vm 2 --vm-bytes 256m
```
