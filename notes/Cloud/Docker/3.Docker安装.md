...
- docker加速配置：[镜像加速器🚀](https://service-crproxy.onrender.com/main/)
...
- **简介：** Podman（POD Manager）是由 Red Hat 开发的下一代容器运行时，是 RHEL 8/9 默认的容器工具，也是 RHCSA/RHCE 2026 考试的官方指定工具。
- **特点：**
  - **无守护进程（Daemonless）**：直接通过 runc 运行容器，不需要 Docker 那种常驻后台进程，部署更轻量、启动更快、故障域更小。
  - **Rootless 原生支持**：普通用户无需 root 权限就能运行容器，安全性更高（这也是考试场景的核心要求）。
  - **完全兼容 Docker CLI**：`alias docker=podman` 即可无缝切换；同时支持 Docker 镜像（`docker.io`）和 Podman 默认（`localhost`）。
  - **原生支持 Pod 概念**：除了单容器，还能管理 Pod（一组共享网络/存储的容器），这是和 K8s 的 Pod 概念对齐的。
  - **Systemd 集成**：用 `podman generate systemd` 一键把容器转成 systemd 服务，开机自启很方便。
- **适用场景：** 适用于 RHEL/CentOS 环境的无守护进程容器运行、rootless 安全容器、以及需要和 systemd 深度集成的生产场景；同时也是 RHCSA/RHCE 2026 改版后的**官方考试工具**。

## Podman vs Docker 对比

| 对比项              | Docker                          | Podman                                           |
| ------------------- | ------------------------------- | ------------------------------------------------ |
| **开发方**          | Docker Inc.（2013）             | Red Hat（2018）                                  |
| **架构**            | C/S 架构（需 daemon `dockerd`） | **无守护进程**（直接通过 runc 运行）             |
| **Rootless 支持**   | 需额外配置（rootless 模式）     | **原生支持**（推荐用法）                         |
| **CLI 兼容性**      | 自己的命令                      | **完全兼容 Docker CLI**（`alias docker=podman`） |
| **镜像兼容性**      | docker.io/xxx                   | 同时支持 docker.io 和 localhost（默认）          |
| **配置文件**        | `/etc/docker/daemon.json`       | `/etc/containers/registries.conf`                |
| **镜像构建**        | `docker build`（Dockerfile）    | `podman build`（Containerfile 或 Dockerfile）    |
| **Pod 支持**        | ❌ 单容器管理                    | ✅ 原生支持（一组容器共享网络/存储，对齐 K8s）    |
| **Systemd 集成**    | 需手写 unit 文件                | ✅ `podman generate systemd` 一键生成             |
| **资源占用**        | daemon 常驻占内存               | 无 daemon，更轻量                                |
| **故障域**          | daemon 挂了所有容器完蛋         | 进程级隔离，单容器故障不影响其他                 |
| **安全模型**        | root daemon（攻击面大）         | **Rootless 默认**，最小权限                      |
| **RHEL 推荐**       | ❌（已被替换）                   | ✅ **RHEL 8/9 默认工具**                          |
| **RHCSA/RHCE 考试** | 旧版考试用                      | ✅ **2026 改版后官方指定**                        |
| **Compose 兼容**    | docker-compose 原生             | podman-compose（兼容大部分语法）                 |
| **K8s 集成**        | 较间接                          | ✅ 更友好（Pod 概念原生对齐）                     |
